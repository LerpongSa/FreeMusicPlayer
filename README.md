...
- **Playlist** — Add Files / Add Folder, Load/Save เป็น `.m3u8`, Clear,
  ...
  - **Add ISO...** — แปลงเพลงจากไฟล์ `.iso` เป็น `.flac` แล้วเพิ่มเข้า playlist
    ...
    **playlist เดิม (ถ้ามี) จะถูกเสนอให้ Save ก่อนเสมอ แล้ว Clear ทิ้งทันที**
    ที่เลือกไฟล์ `.iso` เสร็จ (ก่อนเริ่มแปลงด้วยซ้ำ) เพราะแทร็กที่แปลงจาก ISO
    จะกลายเป็น playlist ใหม่ล้วน ๆ ไม่ใช่ต่อท้ายของเดิม — `promptSaveCurrentPlaylist()`
    เปิด dialog "Save Playlist" แบบเดียวกับปุ่มบน toolbar ให้ทันที ถ้า playlist
    เดิมไม่ว่าง แต่ถึงกด Cancel ไม่เลือกที่เซฟก็ยัง Clear ต่อไปอยู่ดี (ยืนยันกับ
    ผู้ใช้แล้ว 2026-09-19) — dialog นี้เป็นแค่ทางเลือกกันลืมเซฟ ไม่ใช่ gate ที่บล็อก
    การเริ่ม import
