...
- **Playlist** — Add Files / Add Folder, Load/Save เป็น `.m3u8`, Clear,
  ...
  - **Add ISO...** — แปลงเพลงจากไฟล์ `.iso` เป็น `.flac` แล้วเพิ่มเข้า playlist
    ...
    **playlist เดิม (ถ้ามี) จะถูกเสนอให้ Save ก่อนเสมอ แล้ว Clear ทิ้งและเริ่มแปลง
    ทันที ไม่ว่าจะกด Save จริงหรือ Cancel ก็ตาม** ที่เลือกไฟล์ `.iso` เสร็จ
    (ก่อนเริ่มแปลงด้วยซ้ำ) เพราะแทร็กที่แปลงจาก ISO จะกลายเป็น playlist ใหม่ล้วน ๆ
    ไม่ใช่ต่อท้ายของเดิม — `promptSaveCurrentPlaylist()` เปิด dialog "Save Playlist"
    แบบเดียวกับปุ่มบน toolbar ให้ทันที ถ้า playlist เดิมไม่ว่าง แต่เป็นแค่ทางเลือก
    กันลืมเซฟ ไม่ใช่ gate ที่บล็อกการเริ่ม import — ถึงกด Cancel ไม่เลือกที่เซฟ
    หรือเขียนไฟล์ล้มเหลว ก็ยัง Clear playlist แล้วเริ่มแปลง ISO ต่อไปอยู่ดี
    (ผู้ใช้ขอให้กลับไปเป็นแบบนี้อีกครั้ง 2026-09-19 หลังจากเคยลองเปลี่ยนเป็น
    ยกเลิกทั้งหมดถ้ากด Cancel ไปช่วงสั้น ๆ)
